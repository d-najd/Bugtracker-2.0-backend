...
class ProjectTableController(
	...
	): ProjectTableDtoList = service.getAllByProjectId(projectId, includeIssues)
	...
	fun get(
		...
		@RequestParam includeIssues: Boolean = false,
	): ProjectTableDto = service.get(id, includeIssues)
	...
	): ProjectTableDto = service.create(projectId, dto)
	...
	fun update(
		@PathVariable id: Long,
		@RequestBody dto: ProjectTableDto,
	): ProjectTableDto = service.update(id, dto)
	...
	): ProjectTableDtoList = service.swapTablePositions(fId, sId)

	@DeleteMapping("/{id}")
	@ScopedAuthorize("#id", ScopedEvaluatorType.Table, ScopedPermission.Delete)
	fun delete(
		@PathVariable id: Long,
	): ProjectTableDtoList = service.delete(id)
}
